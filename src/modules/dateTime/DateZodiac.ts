...
export class DateZodiac {
	 ...
	 * @param date 日期
	 */
	static getZodiac(date: Date | DateTime): string;
	/**
	 * 获取星座
	 * @param month 月份
	 * @param day 天
	 ...
	 * @param day 天
	 */
	static getZodiac(monthOrDate: number | Date | DateTime, day?: number): string {
		let month: number;
	...
	static getChineseZodiac(year: number): string {
		return this.CHINESE_ZODIACS[
			(year - 1900 + this.CHINESE_ZODIACS.length * 2000) % this.CHINESE_ZODIACS.length
		];
	}
}
