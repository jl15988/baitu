...
class SSQ {
    //朔修正表
    SB: string = this.decSQ(SUO);
    //气修正表
    QB: string = this.decSQ(QI);
    ...
    Yn = [];
    //朔直线拟合参数
    suoKB = [1457698.231017, 29.53067166, // -721-12-17 h=0.00032 古历·春秋
...
//1807665.420323,29.53059851, //  237-02-12 h=0.00033 晋书·律历志(景初历)
    ...
    //气直线拟合参数
    qiKB = [1640650.479938, 15.21842500, // -221-11-09 h=0.01709 古历·秦汉
    ...
    decSQ(val: string) {
        ...
        val = val.replace(/J/g, '00');
        val = val.replace(/I/g, '000');
        val = val.replace(/H/g, '0000');
        val = val.replace(/G/g, '00000');
        val = val.replace(/t/g, '02');
        val = val.replace(/s/g, '002');
        val = val.replace(/r/g, '0002');
        val = val.replace(/q/g, '00002');
        val = val.replace(/p/g, '000002');
        val = val.replace(/o/g, '0000002');
        val = val.replace(/n/g, '00000002');
        val = val.replace(/m/g, '000000002');
        val = val.replace(/l/g, '0000000002');
        val = val.replace(/k/g, '01');
        val = val.replace(/j/g, '0101');
        val = val.replace(/i/g, '001');
        val = val.replace(/h/g, '001001');
        val = val.replace(/g/g, '0001');
        val = val.replace(/f/g, '00001');
        val = val.replace(/e/g, '000001');
        val = val.replace(/d/g, '0000001');
        val = val.replace(/c/g, '00000001');
        val = val.replace(/b/g, '000000001');
        val = val.replace(/a/g, '0000000001');
        val = val.replace(/A/g, o2 + o2 + o2);
        val = val.replace(/B/g, o2 + o2 + o);
        val = val.replace(/C/g, o2 + o2);
        val = val.replace(/D/g, o2 + o);
        val = val.replace(/E/g, o2);
        val = val.replace(/F/g, o);
        return val;
    }
    ...
    calcY(jd) {
        ...
        this.ZQP1 = this.calc(W - 15.2, '气');
        this.ZQP2 = this.calc(W - 30.4, '气'); //补算二气,确保一年中所有月份的“气”全部被计算在内
...
//-721年至-104年的后九月及月建问题,与朔有关，与气无关
        ...
        if (YY >= -721 && YY <= -104) {
            ...
            for (i = 0; i < 3; i++) {
                yy = YY + i - 1;
                //颁行历年首, 闰月名称, 月建
                if (yy >= -721) ns[i] = this.calc(1457698 - JulianDay.J2000 + int2(0.342 + (yy + 721) * 12.368422) * 29.5306, '朔'), ns[i + 3] = '十三', ns[i + 6] = 2;  //春秋历,ly为-722.12.17
                if (yy >= -479) ns[i] = this.calc(1546083 - JulianDay.J2000 + int2(0.500 + (yy + 479) * 12.368422) * 29.5306, '朔'), ns[i + 3] = '十三', ns[i + 6] = 2;  //战国历,ly为-480.12.11
                if (yy >= -220) ns[i] = this.calc(1640641 - JulianDay.J2000 + int2(0.866 + (yy + 220) * 12.369000) * 29.5306, '朔'), ns[i + 3] = '后九', ns[i + 6] = 11; //秦汉历,ly为-221.10.31
            }
            var nn, f1;
            for (i = 0; i < 14; i++) {
                for (nn = 2; nn >= 0; nn--) if (this.HS[i] >= ns[nn]) break;
                f1 = int2((this.HS[i] - ns[nn] + 15) / 29.5306); //该月积数
                if (f1 < 12) this.ym[i] = this.ym[(f1 + ns[nn + 6]) % 12]; else this.ym[i] = ns[nn + 3];
            }
            return;
        }
...
//名称转换(月建别名)
        for (i = 0; i < 14; i++) {
            var Dm = this.HS[i] + JulianDay.J2000, v2 = this.ym[i]; //Dm初一的儒略日,v2为月建序号
            var mc = ChineseDate.Ym[v2 % 12]; //月建对应的默认月名称：建子十一,建丑十二,建寅为正……
            if (Dm >= 1724360 && Dm <= 1729794) mc = ChineseDate.Ym[(v2 + 1) % 12]; //  8.01.15至 23.12.02 建子为十二,其它顺推
            else if (Dm >= 1807724 && Dm <= 1808699) mc = ChineseDate.Ym[(v2 + 1) % 12]; //237.04.12至239.12.13 建子为十二,其它顺推
            else if (Dm >= 1999349 && Dm <= 1999467) mc = ChineseDate.Ym[(v2 + 2) % 12]; //761.12.02至762.03.30 建子为正月,其它顺推
    ...
    //较高精度气
    qi_high(W) {
        var t = Ephemeris.S_aLon_t2(W) * 36525;
        t = t - JulianDay.dt_T(t) + 8 / 24;
        var v = ((t + 0.5) % 1) * 86400;
        if (v < 1200 || v > 86400 - 1200) t = Ephemeris.S_aLon_t(W) * 36525 - JulianDay.dt_T(t) + 8 / 24;
        return t;
    }

    //较高精度朔
    so_high(W) {
        var t = Ephemeris.MS_aLon_t2(W) * 36525;
        t = t - JulianDay.dt_T(t) + 8 / 24;
        var v = ((t + 0.5) % 1) * 86400;
        if (v < 1800 || v > 86400 - 1800) t = Ephemeris.MS_aLon_t(W) * 36525 - JulianDay.dt_T(t) + 8 / 24;
        return t;
    }
}
