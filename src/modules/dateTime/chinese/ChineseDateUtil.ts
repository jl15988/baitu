...
class ChineseDateUtil {
	 ...
	 * @param day 公历天
	 */
	static solarToLunar(year: number, month: number, day: number): ChineseDate {
		let offset =
			DateUtil.getEpochDay(new DateTime(year, month, day)) -
			DateUtil.getEpochDay(LunarInfo.BASE_DATE);
	 ...
	 */
	static lunarToSolar(chineseYear: number, chineseMonth: number, chineseDay: number): DateTime {
		...
		if (
			(chineseYear === 2100 && chineseMonth === 12 && chineseDay > 1) ||
			(chineseYear === 1900 && chineseMonth === 1 && chineseDay < 31)
		) {
			// 超出了最大极限值
			throw new Error(
				"The date is outside the valid range. Please enter a date between 1900.1.31 and 2100.12.1."
			);
		} else {
...
export default ChineseDateUtil;
