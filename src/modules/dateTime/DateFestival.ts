import { ChineseDateUtil, GanZhi, LunarInfo, SolarTerms } from "../chinesedate";
...
export class DateFestival {
	// 节日
	static LUNAR_FESTIVAL: FestivalInfoType = [
		// 1
		{
			1: [
				[
	...
	static getChineseFestival(
		...
		if (festival1) {
			result.super = [
				...result.super, ...festival1.super,
			];
			result.common = [
				...result.common, ...festival1.common,
			];
			result.other = [
				...result.other, ...festival1.other,
			];
		}
		return result;
	}
	 ...
	 * @private
	 */
	private static doGetChineseFestival(
		year: number,
		chineseMonth: number,
		chineseDay: number
	): FestivalResultType {
		...
		const dayGan = GanZhi.getDayGan(
			solarDate.getFullYear(),
			solarDate.month(),
			solarDate.getDate()
		);
		const dayZhi = GanZhi.getDayZhi(
			solarDate.getFullYear(),
			solarDate.month(),
			solarDate.getDate()
		);
