...
class ChineseDate {
	...
	constructor(
		chineseYearOrDate: number | Date | DateTime,
		chineseMonth?: number,
		chineseDay?: number
	) {
		if (chineseYearOrDate instanceof Date || chineseYearOrDate instanceof DateTime) {
			this.date = new DateTime(chineseYearOrDate);
			const chineseDate = ChineseDateUtil.solarToLunar(
				chineseYearOrDate.getFullYear(),
				chineseYearOrDate.getMonth() + 1,
				chineseYearOrDate.getDate()
			);
	 ...
	 */
	toSolar(): DateTime {
		return ChineseDateUtil.lunarToSolar(this.chineseYear, this.chineseMonth, this.chineseDay);
	}

	toString() {
		return GanZhi.getYearGanZhi(this.date.getFullYear()).concat(
			"年",
			" ",
			this.getMonthTraditionalName(this.chineseMonth),
			"月",
			this.getDayName(this.chineseDay)
		);
	}
}
