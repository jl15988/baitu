...
export class SolarTerms {
	 ...
	 */
	static getTermDay(year: number, index: number) {
	 ...
	 * @param date 日期
	 */
	static getTerm(date: Date | DateTime): string;
	 ...
	 * @param day 公历日
	 */
	static getTerm(year: number, month?: number, day?: number): string;
	 ...
	 * @param day 公历日
	 */
	static getTerm(yearOrDate: number | Date | DateTime, month?: number, day?: number) {
		let year: number;
		...
		if (year >= 1900 && year <= 2100) {
			...
			const segmentTable = [
				termInfoStr.substring(0, 1), termInfoStr.substring(1, 3), termInfoStr.substring(
					3,
					4
				), termInfoStr.substring(4, 6),
			];
			const segmentOffset = (month! & 1) == 1 ? 0 : 2;
			if (day == parseInt(segmentTable[segmentOffset])) {
				return this.terms[segment * 4 + segmentOffset];
			} else {
				return day == parseInt(segmentTable[segmentOffset + 1])
					? this.terms[segment * 4 + segmentOffset + 1]
					: "";
			}
		} else {
	 ...
	 * @param date 日期
	 */
	static getTermInterval(termIndex: number, date: Date | DateTime): number;
	/**
	 * 获取同年份的指定日期与指定节气的间隔（节气日期减指定日期）
	 * @param termIndex 节气，从小寒（1）开始
	 * @param year 年份
	 ...
	 * @param day 天
	 */
	static getTermInterval(termIndex: number, year: number, month?: number, day?: number): number;
	 ...
	 * @param day 天
	 */
	static getTermInterval(
		termIndex: number,
		yearOrDate: number | Date | DateTime,
		month?: number,
		day?: number
	): number {
		let year: number;
		...
		return DateUtil.compare(
			new Date(chineseDate.chineseYear, termMonth - 1, termDay),
